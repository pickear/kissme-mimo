...
<div class="chsm"><span>内容管理</span></div>
<div class="czsrt">
<span>你可以：</span>
...
<span>你可以：</span>
<span class="dist">标题：</span><input name="params[channel]" value="${page.params.channel}" type="hidden" /><input name="params[title]" value="${page.params.title}" type="text" /><input class="tjbtn" type="submit" value="提交" />
</div>
<p>${message.text }</p>
<div class="rcd_td">
  ...
  <c:choose>
  	...
  	<c:forEach items="${page.result}" var="article" begin="0">
				<tr>
					...
					<td>
						<a href="${ctx }/${article.channel.path }/${article.id}" target="_blank" >浏览</a>&nbsp;
						<shiro:hasPermission name="article:edit">
