...
<script type="text/javascript">
  $(function(){
	  ...
	  $("#upload").click(function(){
		  	var $form = $("#uploadForm");
		  	$form.ajaxSubmit({
		  		url : "${ctx}/photo-resource/upload/",
		  		type : "POST",
		  		data : {"pathname" : "${param.pathname}"},
		  		dataType : "json",
		  		success:function(data){
					if(data && data.state && data.state.indexOf("SUCCESS") != -1){
						location.reload();
						return;
					}
...
<div class="chsm"><span>网站图片资源管理</span></div>
<div class="czsrt">
<span>当前目录：<c:choose><c:when test="${not empty param.pathname }">${param.pathname }</c:when><c:otherwise>/</c:otherwise> </c:choose> &nbsp;&nbsp;&nbsp;&nbsp;</span>
<input class="tjbtn" onclick="javascript:history.go(-1);" type="button" value="后退" />
</div>
<div class="czsrt">
<shiro:hasPermission name="photo-resource:upload">
<form id="uploadForm" enctype="multipart/form-data">
<span class="dist">上传文件：</span>
<input id="file" type="file" name="file" size="30" class="ifile" onchange="this.form.filePath.value=this.value.substr(this.value.lastIndexOf('\\')+1);" />
<input id="filePath" type="text" size="30" readonly="readonly" />
<input class="tjbtn" id="review" type="button" value="浏览" onclick="this.form.file.click();" />
<input class="tjbtn" id="upload" type="button" value="上传" />
</form>
</shiro:hasPermission>
</div>

<form id="myForm" action="${ctx }/photo-resource/list/" method="get">
...
<div class="rcd_td">
  ...
  <c:choose>
    ...
    <c:choose>
		...
		<a href="${ctx }/photo-resource/list/?pathname=${codec:urlEncode(bean.path)}"> [目录]&nbsp;${bean.name}</a>
		</c:when>
		<c:otherwise>
		[普通文件]&nbsp;${bean.name}
		</c:otherwise>
