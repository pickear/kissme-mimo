...
public class SecurityResourceObjectController extends ControllerSupport {

	@Autowired
	@Qualifier("security-resource-service")
	...
	private ResourceObject createRootResourceBeanIfNeccessary(ResourceObject bean, String pathname) {
		...
		if (null == bean && StringUtils.isBlank(pathname)) {
			return ResourceObjectFactory.newSecurityResourceObject().setDirectory(true).setPath("/");
		}
	...
	private boolean isUnacceptableFile(MultipartFile file, Conf conf) {
		...
		if (!conf.isAllowedSecurityResourceTypes(fileSuffix)) {
			return true;
		}
	...
	private void doUpload(ResourceObject bean, MultipartFile file, Conf conf) throws IOException {
		String targetFilePath = Files.join(conf.getSecurityResourcePath(), bean.getPath(), file.getOriginalFilename());
