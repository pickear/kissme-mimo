	...
	$(function(){
		...
		$("#del").click(function(){
			...
			if(confirm("你确定要删除这些内容吗?")){
				...
				$("#myForm").attr("action", "${ctx}/guestbook/delete/")
							.attr("method","post")
...
<div class="chsm"><span>站点留言管理</span></div>
<div class="czsrt">
<span>你可以：</span>
<shiro:hasPermission name="guestbook:delete">
<input class="tjbtn" id="del" type="button" value="删除" />
</shiro:hasPermission>
<span class="dist">名称：</span><input name="params[author]" value="${page.params.author}" type="text" /><input class="tjbtn" type="submit" value="提交" />
</div>
<p>${message.text }</p>
<div class="rcd_td">
  ...
  <c:choose>
  	...
  	<tr>
    <td style="border-left:0;"><input type="checkbox" name="items" id="checkbox" value="${guestbook.id}"/></td>
    <td>${guestbook.author}${guestbook.admin == 'true' ? '[管理员]' : '[匿名用户]'}&nbsp;</td>
    <td>${guestbook.email}&nbsp;</td>
    <td>${guestbook.status == 1 ? '[正常]&nbsp;' : '[审核中]&nbsp;'}&nbsp;</td>
    <td>${fn:substring(guestbook.content,0,15)}&nbsp;</td>
    <td class="date">${guestbook.createTime}&nbsp;</td>
    <td>
    	<shiro:hasPermission name="guestbook:edit">
